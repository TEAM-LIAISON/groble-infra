...
resource "aws_codedeploy_deployment_group" "groble_dev_deployment_group" {
  app_name               = aws_codedeploy_app.groble_app.name
  ...
  ecs_service {
    cluster_name = aws_ecs_cluster.groble_cluster.name
    service_name = "${var.project_name}-dev-service"  # 실제 서비스 이름 사용
  }

  # Blue/Green 배포 설정 (ECS 전용, 개발환경은 더 빠르게)
  blue_green_deployment_config {
    # 배포 준비 옵션
    deployment_ready_option {
      action_on_timeout = "CONTINUE_DEPLOYMENT"
  ...
  load_balancer_info {
    target_group_pair_info {
      target_group {
        name = aws_lb_target_group.groble_dev_blue_tg.name
      }

      target_group {
        name = aws_lb_target_group.groble_dev_green_tg.name
      }
  ...
  # ECS 서비스가 먼저 생성되도록 의존성 설정
  depends_on = [
    aws_ecs_service.groble_dev_service
  ]
}
