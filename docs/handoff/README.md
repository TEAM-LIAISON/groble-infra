...
| [rolling-deploy-prerequisites.md](./rolling-deploy-prerequisites.md) | groble-backend | 2026-08-30 | 🔄 진행 중 | **[Phase 6](../runbook/phase-06-deployment-controller.md) 착수 조건 — 2026-09-01 회신·회답 완료. 4건 → 5건으로 늘었다.** graceful shutdown 은 **이미 적용돼 있었다**(`graceful`/`20s`). 드레이닝 값은 **dereg 60 / graceful 20 / stopTimeout 90** 으로 확정(결제 승인 상한 30초 · executor 종료 대기). readiness 분리는 **인프라 선행 리팩터**가 필요하다 — 타깃그룹 4개가 헬스체크 경로 변수를 공유해 dev 만 못 바꾼다. **남은 대기: 앱 SIGTERM 실측 · 스케줄러 23개 전수 점검**(새 조건, 가장 오래 걸린다) |
