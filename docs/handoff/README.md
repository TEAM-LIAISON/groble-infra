...
| [rolling-deploy-prerequisites.md](./rolling-deploy-prerequisites.md) | groble-backend | 2026-08-30 | 🔄 진행 중 | **[Phase 6](../runbook/phase-06-deployment-controller.md) 착수 조건 4건 — 2026-09-01 회신 도착.** graceful shutdown 은 **이미 적용돼 있었다**(`graceful`/`20s`). readiness 분리는 가능하나 **인프라 쪽 3건(ALB 경로 → readiness · ECS healthCheck → liveness · WAF 예외 확대)이 세트로 필요**하다. 드레이닝 값에 이견 — **dereg 60 / stopTimeout 90** 제안(결제 승인 상한 30초 · executor 종료 대기가 graceful 20초 뒤에 붙는다). 추가로 **배치 23개에 분산 락이 없어** 앱 측 전수 점검이 새 차단 조건이 됐다 |
