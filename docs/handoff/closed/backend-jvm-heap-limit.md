...
| **작성일** | 2026-08-18 |
| **상태** | 📦 **종결** (2026-08-20) — PR #826 머지, dev·prod 배포 완료. 힙 상한 2,878 → **900 MiB** 확인 |
| **긴급도** | 높음 — 인프라 Phase 8(ASG 전환)의 차단 조건이었다 |
| **영향 범위** | prod·dev API 컨테이너 양쪽 |

> 📦 **종결된 요청서다.** 아래는 요청 당시의 근거·측정 기록이며, 요청한 변경은 이미 반영·배포됐다.
> 후속 재측정 결과는 [Phase 2 런북](../../runbook/phase-02-observability.md#prod-재측정-2026-08-24--memoryreservation-확정)에 있다.
...
지금은 노드의 스왑파일이 이를 흡수해 죽지 않고 있으나, **인프라 Phase 8에서 스왑이 사라지면 prod API가 OOM으로 종료됩니다.**
...
## 3. 왜 지금 고쳐야 하나 — Phase 8 차단 조건

인프라는 현재 단일 EC2(pet) → **ASG 기반 다중 인스턴스(cattle)** 구조로 이관 중입니다 (Phase 8).

- 지금 OOM을 막고 있는 스왑파일은 **AMI 기본 기능이 아니라 현재 노드의 `user_data` 스크립트가 만드는 것**입니다.
- Phase 8에서 노드는 **ECS-optimized AL2023 AMI + Launch Template**으로 교체되며, 이 스왑파일은 사라집니다.
- **JVM 설정을 그대로 둔 채 노드를 교체하면, 새 ASG 노드에서 prod API가 OOM kill로 종료됩니다.**
- 하필 마이그레이션 도중이라 원인 판별이 가장 어려운 시점입니다.

추가로, 이 값이 고쳐지기 전에는 인프라가 **Phase 8/8의 `memoryReservation`(태스크 배치 기준값)을 확정할 수 없습니다.**
...
> 인프라의 노드 사이징(Phase 8/8) 재검토로 번지지 않습니다.
...
| `groble-infra/docs/plan/infra-ha-improvement-plan.md` §2.1 | 트래픽·자원 기준선 |
