...
| | |
|---|---|
| **상태** | **구현·배포 전부 완료.** 남은 것은 시간이 지나야 확인되는 2건과 백엔드 대기 1건뿐 — 아래 [완료 조건](#phase-2-완료-조건) 참조 |
...
**구현과 배포는 전부 끝났다.** 남은 3건은 작업이 아니라 **기다림**이다.

| 남은 것 | 언제 확인 가능 | 막고 있는 것 |
|---|---|---|
| **R6·R7 오탐 없음 확인** (배포 1회 후 24시간) | **2026-08-21 저녁** — 마지막 배포 2026-08-20 17:1x | 없음. 다만 이게 기존 규칙의 최대 결함이었으므로 확인 전에는 "고쳤다"고 말할 수 없다 |
| **prod 2~3일 재측정** → `memoryReservation` 확정 | **2026-08-22~23** — JVM 수정 배포 2026-08-20 16:0x | 🔴 **[Phase 7](./phase-07-prod-asg.md) 차단 조건** |
| **R10~R14** (백엔드 지표 3종) | 백엔드 배포 순서대로 | 없음. Phase 2 완료를 막지 않는다 — 별도 레포 의존이므로 [요청서](../handoff/payment-alerts-review.md) §6 으로 넘긴다 |

> 📌 **Phase 3~6 은 지금 시작해도 된다.** 위 3건 중 Phase 를 막는 것은 `memoryReservation` 하나뿐이고,
> 그것은 **Phase 7** 의 선행 조건이다. Phase 2 의 목적("ASG 도입 전에 관측 사각지대를 없앤다")은
> 노드 스크레이프가 `ec2_sd` 로 바뀌고 알림 14건이 가동되면서 이미 달성됐다.
