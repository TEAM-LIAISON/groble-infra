...
| | |
|---|---|
| **상태** | 🔄 진행 중 — 배포·검증 완료 (2026-08-24), **백엔드 회신 대기 2건**. `memoryReservation` 확정으로 **[Phase 9](./phase-09-prod-asg.md) 차단 해제.** 남은 것은 ① 죽은 JVM 힙 알람 수정 ② 백엔드 지표 후 R10~R14 |
...
> **더 이상 지금 할 수 있는 작업이 없다** — 남은 3건은 전부 기다려야 하는 것이다. 아래 [완료 조건](#phase-2-완료-조건--2026-08-24-시점-대기-2건-모두-통과).
...
| 건 | 상태 | 필요한 결정 |
|---|---|---|
| **prod JVM 힙 수정 배포** | ✅ **완료** (2026-08-20). 힙 상한 2,878 → **900 MiB** 확인 | 남은 것은 **2~3일 뒤 재측정**뿐이다 → [Phase 9](./phase-09-prod-asg.md)(prod)·[7](./phase-07-dev-cache-asg.md)(dev) `memoryReservation` 확정. **[Phase 9](./phase-09-prod-asg.md) 차단 조건** |
| **`JVM 힙 상한 > 컨테이너 리밋` 알람** | ✅ **해소.** prod 배포로 900 MiB < 리밋 1,500 MiB 가 되어 조건이 성립하지 않는다 | 없음 |
| **`컨테이너 메모리 하드리밋 근접` 알람 발화 중** | **dev-mysql 99.1%** (253.8/256 MiB) · prod-api 92.9% | ✅ **결정됨 — 상향하지 않는다.** [Phase 5](./phase-05-dev-rds.md) 에서 dev MySQL 이 RDS 로 이관되며 컨테이너째 사라지므로 그때까지 발화를 감수한다. 조사 결과(재시작 5회 증거 · `ignore_changes` 함정 · 무효한 `MYSQL_INNODB_BUFFER_POOL_SIZE`)는 [Phase 5](./phase-05-dev-rds.md) 문서에 기록했다 |
...
| **prod 재측정** → `memoryReservation` | ✅ **확정** — 아래 [prod 재측정](#prod-재측정-2026-08-24--memoryreservation-확정). 🔓 **[Phase 9](./phase-09-prod-asg.md) 차단 해제** |
...
| 대상 | 값 | 근거 |
|---|---|---|
| [Phase 9](./phase-09-prod-asg.md) prod `memory_reservation` | **1,300 MiB** | RSS p99 1,281 · 최대 1,290 을 덮는 최소값 |
| prod `memory` (하드리밋) | **1,500 MiB 유지** | 여유 210 MiB. 상향 불필요 |
| [Phase 7](./phase-07-dev-cache-asg.md) dev `memory_reservation` / `memory` | **prod 와 동일** | dev RSS p99 **1,277** · 최대 **1,287 MiB** — prod 와 사실상 같다 |
...
- [x] prod 재측정 → **`memory_reservation` = 1,300 MiB 확정** (2026-08-24) — [Phase 9](./phase-09-prod-asg.md) 차단 해제
- [ ] 🔴 **JVM 힙 알람 수정** — `id="G1 Old Gen"` 시계열 소멸로 NoData(=OK) 상태. 어떤 오설정에도 안 울린다
- [x] dev-mysql 메모리 리밋 결정 — **상향하지 않고 [Phase 7](./phase-07-dev-cache-asg.md) 이관에 맡긴다**
- [ ] `spring-apps` 태스크 단위 스크레이프 (Cloud Map) — **[Phase 9](./phase-09-prod-asg.md) 로 이관됨**, `desired_count` 2 이상 전에 필수
...
| [Phase 9](./phase-09-prod-asg.md) `memory_reservation` | ✅ **1,300 MiB 확정** (2026-08-24 재측정, RSS p99 1,281 / 최대 1,290) |
| [Phase 7](./phase-07-dev-cache-asg.md) dev `memory` | ✅ **prod 와 동일** (dev RSS p99 1,277 / 최대 1,287). 둘 다 `-Xmx900m` 이라 RSS 는 라이브 셋이 아니라 힙 상한이 결정한다 |
| t3.medium 노드당 태스크 2개 수용 | 수용 가능할 전망 (2 × ~1,300 MiB + 노드 오버헤드 < 3,837 MiB). 어차피 **ENI 3개 제약으로 노드당 awsvpc 태스크는 최대 2개**가 상한이다 |

### ⚠️ [Phase 9](./phase-09-prod-asg.md) 차단 조건

**지금 OOM을 막고 있는 스왑파일은 AMI 기능이 아니라 현재 노드의 `user_data`가 만드는 것이다.**
[Phase 9](./phase-09-prod-asg.md)에서 노드가 ECS-optimized AL2023 AMI + Launch Template으로 교체되면 이 스왑은 사라진다.
...
3. ✅ `groble-images`의 Prometheus config를 `static_configs` → `ec2_sd_config`로 변경
   ...
   - ⚠️ 단 `node_type` / `instance_id` / `availability_zone` 3개가 **추가되므로 라벨 집합이 바뀌어 전환 시점에 시계열이 한 번 갈라진다.**
     라벨 매처는 부분 일치라 기존 쿼리·대시보드는 그대로 동작한다. [Phase 9](./phase-09-prod-asg.md)에서 필요한 라벨이므로 **사용자 영향이 없는 지금** 끊는 편이 낫다고 판단했다
4. ~~CI에서 `promtool check config` 게이트 추가~~ — ✅ **이미 있었다.** `.github/workflows/build.yml`의 Validate config 스텝
5. ⏭ **"기대 타깃 수 미달" 알람** → **Grafana 알림으로 결정. [2-2](#2-2-grafana-프로비저닝-as-code--배포-완료)에서 프로비저닝한다**
...
⚠️ 이 값들은 **노드 수가 바뀌면 함께 바꿔야 한다** ([Phase 7](./phase-07-dev-cache-asg.md)·[9](./phase-09-prod-asg.md) 의 ASG desired 변경 시).
...
**성립 조건 — 태그 전파.** [Phase 7](./phase-07-dev-cache-asg.md)·[9](./phase-09-prod-asg.md)의 ASG/Launch Template에서 `Cluster`·`environment`·`Name`·`Type` 태그가
...
[Phase 9](./phase-09-prod-asg.md)의 선행 항목으로 이관.
...
| 노드 스왑 | > 300 MiB | warning | CloudWatch 는 EC2 스왑을 수집 안 함. **[Phase 9](./phase-09-prod-asg.md) 차단 조건** |
...
- [x] ([Phase 9](./phase-09-prod-asg.md) 진입 전) prod 재측정 → `memory_reservation` = **1,300 MiB** 확정 (2026-08-24)
...
> ⚠️ **이 Phase를 건너뛰고 [Phase 7](./phase-07-dev-cache-asg.md)·[9](./phase-09-prod-asg.md)로 가지 않는다.** 새 ASG 노드가 스크레이프되지 않는 상태로 마이그레이션을 진행하면, 문제가 생겨도 지표가 없다.
>
> ⚠️ **JVM 힙 상한 수정 없이 [Phase 9](./phase-09-prod-asg.md)로 가지 않는다.** 스왑이 사라지면서 prod API가 OOM kill된다 (2-0 참조).
