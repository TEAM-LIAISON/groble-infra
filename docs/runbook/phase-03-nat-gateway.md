...
| **시점** | 저트래픽 시간대, **배포가 없는 시간대**, 그리고 **[RDS 8.4 전환](./adhoc/rds-mysql-84-upgrade.md) 완료 이후** (아래 3-d) |
| **되돌리기** | 스위치 되돌리기 (급하면 CLI 한 줄) |
| **작업 브랜치** | `feat/phase-03-nat-gateway` |
...
> ⛔ **선행 조건: [RDS MySQL 8.4 전환](./adhoc/rds-mysql-84-upgrade.md)이 끝난 뒤에 한다.**
> (2026-08-28 결정)
>
> *(위 링크는 `feat/rds-mysql-84-upgrade` 브랜치에서 들어오는 문서다. 두 브랜치가 main 에
> 머지되기 전까지 이 브랜치에서는 열리지 않는다.)*
>
> 두 작업의 blast radius 가 결제 경로에서 겹친다 — 이쪽은 앱 → PG(페이플) **아웃바운드**를
> 갈아끼우고, RDS 전환은 결제 **쓰기**를 1분간 막는다. **같은 날 하면 결제가 깨졌을 때
> 원인이 NAT 인지 DB 인지 가릴 수 없다.** 백엔드가 RDS 전환에 "앱 배포 없는 날" 조건을
> 건 것과 같은 이유다.
>
> **3-c(NAT Gateway 생성)까지는 먼저 해도 된다.** 라우트 테이블에 연결하지 않는 한
> 트래픽은 바뀌지 않는다. **선을 긋는 지점은 `use_nat_gateway = true` 다.**
