...
→ **이 Phase 에서는 임시로 `100 / 200` 을 쓰고, Phase 7(dev)·9(prod) 에서 desired 를 2로 올릴 때 To-Be 값으로 바꾼다.**
...
- [ ] `min/max` 가 **임시값 `100/200`** 인지, Phase 7(dev)·9(prod) 에서 To-Be 값으로 되돌릴 항목으로 인계되었는지
