...
| **되돌리기** | ~~구 인스턴스(`-old1`) 로 엔드포인트 되돌리기~~ — **구 인스턴스를 2026-08-29 삭제해 불가능하다.** 최종 스냅샷 `groble-prod-mysql-80-final` 만 남아 있다 |
| **겹치는 Phase** | **없음** — RDS 인스턴스만 건드린다. [8](../phase-08-prod-elasticache.md)(Redis) · [9](../phase-09-prod-asg.md)(EC2 노드) 와 자원이 겹치지 않아 언제 끼워 넣어도 됐다 |
| **실행일** | **2026-08-29 02:18 KST** (예정 창 04:10~04:50 을 담당자 판단으로 앞당겼다 — 백엔드 인지 상태, 03:00 배치까지 44분 여유) |
...
> 어떻게 만들어졌는지와 **다음 DB 전환에서 반복하지 말아야 할 것**을 남겨둔 것이다.

**한 일**

| | |
|---|---|
| 방식 | RDS Blue/Green 배포로 **MySQL 8.0.45 → 8.4.11** |
| 대상 | `groble-prod-mysql` (db.t3.micro, 단일 AZ 2c) |
| 파라미터 그룹 | `8.4-params` (in-sync) |
| 구 인스턴스 | `groble-prod-mysql-old1` — **2026-08-29 16:09 KST 삭제** (예정 8/31 보다 이틀 앞당김) |
| 남은 것 | 최종 스냅샷 **`groble-prod-mysql-80-final`** (8.0.45, 20GB, 암호화) |
| 목적 달성 | 확장 지원 과금 **월 $178.56 → 계량기 정지** (이미 발생한 $143 은 소급되지 않는다) |

**사용자 영향 — 계획보다 컸다**

| | 계획 | 실제 |
|---|---|---|
| 쓰기 차단 | 약 1분 | **35초** (02:18:55 → 02:19:30) ✅ |
| 그 외 | 무중단 | ❌ **02:19:30~02:27, 약 7~8분간 쓰기 실패** (사고 1) |

**⭐ 가장 중요한 교훈 — 다음 DB 전환에 그대로 적용된다**

1. **전환 직후 앱을 재기동한다.** 앱 커넥션 풀이 구 DB 를 붙잡는다.
   원인은 DNS 캐시가 아니라 **HikariCP `max-lifetime` 30분**이다 — 재배포 없이 뒀다면
   최대 30분 뒤에야 회복됐을 사건이다. TTL 을 낮춰도 재발한다
   ([정정 회신](../../handoff/closed/jvm-dns-cache.md))
2. **헬스체크 200 으로 전환 성공을 판정하면 안 된다.** 구 DB 는 `read_only=1` 이라 조회는
   되고 쓰기만 실패했다. 커넥션 풀의 유효성 검사도, ALB 헬스체크도 이걸 정상으로 봤다.
   **커넥션이 어느 인스턴스에 붙어 있는지를 직접 세야 한다**
3. **`engine_version` 은 마이너까지 고정한다.** `"8.4"` 로 두면 AWS 가 패밀리 기본값(8.4.9)으로
   해석해 **다운그레이드를 시도하고 apply 가 실패한다** (사고 2)
4. **Blue/Green 전환은 state 재연결이 필요하다** — 인스턴스 식별자가 바뀌므로 코드 수정만으로는
   안 된다 (`db-WM4VK…` → `db-CY5YZ…`)

**검증 결과**

- 앱 커넥션 신 DB **10 / 구 DB 0**, 삭제 전 6시간 동안 30분 간격 12회 전부 0
- RDS 알람 6개 전부 신 인스턴스를 보고 있고 전부 `OK`
- `terraform plan` No changes · 운영 DB 삭제 방지 `True` 유지
- 앱 헬스체크 200 (구 인스턴스 삭제 구간 내내 유지)

**아직 확인하지 못한 것**

- [ ] **확장 지원 과금 $0** — CE 반영에 1~2일 걸린다. 아래 [남은 것](#남은-것) 의 명령으로 확인
- [ ] **전환 구간(02:19~02:27) 결제 점검** — 그 8분간 실패한 쓰기가 무엇이었는지.
      구 DB 가 `read_only` 라 **거부된 것이지 구 DB 로 흘러가지 않았다** — 유실이 아니라
      실패이며, 재시도 여부는 앱 측 확인이 필요하다
- [ ] **첫 09:00 배치 정상 동작** 확인

**롤백**

**불가능하다.** 구 인스턴스를 삭제했다. 최종 스냅샷 `groble-prod-mysql-80-final` 로
새 인스턴스를 복원하는 것만 가능하며, 그것은 **02:19:30 시점에 얼어붙은 사본**이다.
...
| 조건 | 누가 | 반영 위치 |
|---|---|---|
| ① 헬스체크 임시 완화를 **전환 전에 확정** | 인프라 | [§4](#4--반드시-짚어야-할-위험--alb-헬스체크가-태스크를-죽인다) |
...
> 해당 변경은 아래 [Terraform 정합화](#terraform-정합화--state-재연결이-필요하다-코드-수정만으로는-안-된다) 에 명세만 해 뒀고,
...
1. **ALB 임계 완화** ([§4](#4--반드시-짚어야-할-위험--alb-헬스체크가-태스크를-죽인다) 명령 실행)
2. 복제 지연 0 재확인
3. 스위치오버
