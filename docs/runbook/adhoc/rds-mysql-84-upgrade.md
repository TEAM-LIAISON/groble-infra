...
| | |
|---|---|
| **상태** | ✅ **전환 완료 (2026-08-29 02:19 KST).** state 재정렬까지 끝. 남은 것은 **D+7 구 인스턴스 삭제**와 과금 정지 확인 — 아래 [실행 기록](#실행-기록-2026-08-29) |
...
| ③ | 백엔드 회신 3건 ([요청서](../../handoff/rds-mysql-84-compatibility.md)) | ✅ 통과 — 3건 모두 진행 가능, 조건 3개 첨부 |
...
요청서: [`docs/handoff/rds-mysql-84-compatibility.md`](../../handoff/rds-mysql-84-compatibility.md)
...
- [ ] 전환 직후 — **승인 불명 대기열 조회**, 결제 알림 채널 확인, 에러율·슬로우 쿼리 대시보드 확인
- [ ] 전환 후 첫 **09:00 — 상품 정기결제 청구 배치** 정상 수행 확인

> 백엔드의 09:00 배치 확인이 끝나야 전환이 진짜 끝난 것이다. **롤백 창(D+7)을 그전에 닫지 말 것.**
...
- DNS 는 정상이었다 (`groble-prod-mysql` → 10.0.12.96 = 신규). **JVM 이 구 IP 를 캐시**한 것이다
...
**다음에 할 일** — 이 문제는 예측했는데 **막을 준비를 안 해 둔 것이 실수다.**
전환 직후 **곧바로 앱을 재배포하는 것을 절차에 넣거나**, 백엔드에 JVM
`networkaddress.cache.ttl` 을 짧게 설정하도록 요청할 것.
...
1. **전환 후 첫 09:00** — 백엔드가 상품 정기결제 청구 배치 정상 수행 확인
2. **02:19~02:27 구간의 결제 점검** — 백엔드에 승인 불명 대기열 확인 요청.
   로그에서 `1290 (HY000): ... --read-only option` 으로 식별된다
3. **구 인스턴스 삭제 — 2026-08-31 예정** ([정리 절](#정리--2026-08-31-예정-롤백-창이-닫힌-뒤)).
   원래 D+7 로 잡았으나 **2일 경과 관찰 후 8/31 로 앞당긴다** (담당자 결정, 2026-08-29).
   근거 두 가지:
   - **롤백 가치는 전환 직후 몇 시간이면 사실상 소멸한다.** 되돌리면 전환 후 쌓인
     결제·주문 데이터를 전부 잃으므로, 하루만 지나도 롤백은 현실적인 선택지가 아니다
   - **`-old1` 은 8.0 이라 살아 있는 동안 확장 지원 과금이 계속 붙는다**(하루 $5.76).
     9/5 대신 8/31 에 지우면 약 $29 를 아낀다
4. **과금 정지 확인** — CE 반영에 1~2일. 8/30 이후
   `APN2-ExtendedSupport:Yr1-Yr2:MySQL8.0` 이 $0 인지 확인
